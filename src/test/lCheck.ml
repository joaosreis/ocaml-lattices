...
let rec split i ls =
  match (i, ls) with
  | 0, ls -> ([], ls)
  | _n, [] -> failwith "splitting beyond list length"
  | n, l :: ls ->
      let ls, rs = split (n - 1) ls in
      (l :: ls, rs)
...
let op_monotone (type a b) (module PL : LATTICE_TOPLESS with type elem = a)
...
let op_invariant (type a b) (module PL : LATTICE_TOPLESS with type elem = a)
    (module RL : LATTICE_TOPLESS with type elem = b) k =
  k
    ( Print.pair PL.to_string PL.to_string,
      PL.equiv_pair,
...
let op_strict (type a b) (module PL : LATTICE_TOPLESS with type elem = a)
    (module RL : LATTICE_TOPLESS with type elem = b) k =
  k
    ( PL.to_string,
      always PL.bot,
      (fun op bot ->
        assume (PL.eq bot PL.bot);
        RL.eq (op bot) RL.bot),
      "strict",
      1 )

let op_distributive (type a b) (module PL : LATTICE_TOPLESS with type elem = a)
    (module RL : LATTICE_TOPLESS with type elem = b) k =
...
let ( ---> ) (type e e') a (b : (module LATTICE_TOPLESS with type elem = e)) k =
  a (fun (l, optranl, r, optranr, prop) ->
      let module R = (val r : LATTICE_TOPLESS with type elem = e') in
      (* manual "upcast" *)
      let r = (module R : ARB_ARG with type elem = e') in
      (* the module type *)
      k
        ( l,
...
let ( -<-> ) (type e) a (b : (module LATTICE_TOPLESS with type elem = e)) k =
...
module MkListLattice (A : LATTICE_TOPLESS) = struct
  ...
  let arb_elem_le vs =
    let gen =
      Gen.(
        int_bound (1 + List.length vs) >>= fun i ->
        let smaller_vs, _ = split i vs in
        List.fold_right
          (fun v acc_gen ->
            map2 (fun v a -> v :: a) (A.arb_elem_le v).gen acc_gen)
          smaller_vs (return []))
    in
